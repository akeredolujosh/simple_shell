...
/**
  * input_buf - create a buffer command chain
	...
	if (!*len) /* if nothing is left in the buffer, fill it */
	{
		...
		*buf = NULL;
		...
		if (r > 0)
		{
			if ((*buf)[r - 1] == '\n')
			{
				(*buf)[r - 1] = '\0'; /* remove any trailing newline */
				r--;
			}
...
/**
  * get_input - line minus a new line
  * @info: struct parameter
  * Return: bytes read
  */
ssize_t get_input(info_t *info)
{
	...
	{
		j = i; /* initialize a new iterator to current buf position */
...
/**
  ...
  * @info: struct parameter
  * @ptr: addr of pointer to buffer, NULL or preallocated
  * Return: s
  */
